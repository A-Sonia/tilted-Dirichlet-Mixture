...
source("~/Library/Mobile Documents/com~apple~CloudDocs/tDMJune25/simulation0825/short/versionA.R")
 source("~/Library/Mobile Documents/com~apple~CloudDocs/tDMJune25/FinancialData/untilted/checks.R")
...
We replace the tilted Dirichlet mixture distribution with an unconstrained Dirichlet mixture distribution and run the same the MCMC algorithm (for only $10000$ iteration) on the financial dataset introduced in the paper.
...
choices=rbind(
    c( 'PEAR','2)Phat',as.numeric(uniqueResults_thin[which.min(uniqueResults_thin$PEAR),c('iter','K','PEAR')]))
  )
